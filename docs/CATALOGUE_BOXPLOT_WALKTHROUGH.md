...
The catalogue comparison uses gWFM, ISC-EHB and gCMT earthquake depths. The
plotting code is in `catalogue_distribution_plots.py`. It reads the compact
results table produced by the main analysis, so it does not rerun the
ground-motion or vulnerability calculations.
...
- `source_depth_km`, the catalogue depth used by the model;
- `maximum_pga_g`, the highest site-level median PGA among the 311 receivers;
- `maximum_structural_loss_ratio`, the highest mean structural loss ratio
  among the same receivers.
...
earthquakes, keeps the comparison balanced. The plotted values, including
`source_depth_km`, are saved in
...
Source depth is plotted separately on a linear vertical axis. A dashed 30 km
line marks the shallow-crustal depth limit of the Akkar et al. (2014) GMPE
database used by this project. The depth figure uses the same balanced set of
90 earthquakes as the PGA/loss figure.
...
Then generate both figures and their supporting data:
...
- `outputs_gwfm/depth_sensitivity_analysis/catalogue_depth_boxplot.png`
- `outputs_gwfm/depth_sensitivity_analysis/catalogue_boxplot_event_maxima.csv`
- `outputs_gwfm/depth_sensitivity_analysis/catalogue_boxplot_summary.csv`

The summary CSV records depth Q1, median, Q3, IQR, mean, maximum depth and the
number of events deeper than 30 km for each catalogue, alongside the existing
PGA and structural-loss statistics. These values can be used to check the
figures without reading measurements from the images.

For the current 90-event balanced sample, the depth IQRs are 24.25 km for
gWFM, 29.00 km for ISC-EHB and 17.75 km for gCMT.
